...
public class DailyItemsPanel extends JPanel {
    ...
    void jbInit() throws Exception {
        ...
        currentDateLabel.setText(CurrentDate.getDate().getFullDateString());
...
//            	Util.debug("current project is " + CurrentProject.get().getTitle());
            	...
            	currentNote = CurrentProject.getNoteList().getNoteForDate(CurrentDate.getDate());
...
//                // DEBUG
        ...
        editorPanel.setDocument(currentNote);
        History.add(new HistoryItem(CurrentDate.getDate(), CurrentProject.get()));
        ...
        updateIndicators(CurrentDate.getDate(), CurrentProject.getTaskList());
...
//		Util.debug("currentNoteChanged");
		...
		currentNote = note;
		editorPanel.setDocument(currentNote);
        calendar.set(CurrentDate.getDate());
...
//		Util.debug("currentProjectChanged");
        ...
        App.getFrame().setCursor(waitCursor);
        if (!changedByHistory)
            History.add(new HistoryItem(CurrentDate.getDate(), newprj));
        if (editorPanel.isDocumentChanged())
            saveNote();
        /*if ((currentNote != null) && !changedByHistory && !addedToHistory)
                    History.add(new HistoryItem(currentNote));*/
        ...
        updateIndicators(CurrentDate.getDate(), tl);
        App.getFrame().setCursor(cur);
    }
    ...
    public void selectPanel(String pan) {
       ...
       //     calendar.jnCalendar.updateUI();
        ...
        if (isAg)
        	agendaPanel.refresh(CurrentDate.getDate());
