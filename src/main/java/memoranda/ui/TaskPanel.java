    ...
    scrollPane.getViewport().add(taskTable, null);
        ...
        CurrentProject.addProjectListener(new IProjectListener() {
            public void projectChange(IProject p, INoteList nl, ITaskList tl, IResourcesList rl) {
                newTaskB.setEnabled(
                    CurrentDate.getDate().inPeriod(p.getStartDate(), p.getEndDate()));
            }
...
//		CurrentProject.getTaskList().adjustParentTasks(t);
    ...
    void newTaskB_actionPerformed(ActionEvent e) {
        ...
        Dimension frmSize = App.getFrame().getSize();
        Point loc = App.getFrame().getLocation();
        dlg.startDate.getModel().setValue(CurrentDate.getDate().getDate());
        dlg.endDate.getModel().setValue(CurrentDate.getDate().getDate());
        dlg.setLocation((frmSize.width - dlg.getSize().width) / 2 + loc.x, (frmSize.height - dlg.getSize().height) / 2 + loc.y);
        dlg.setVisible(true);
        if (dlg.CANCELLED)
            return;
...
//        Util.debug("Adding sub task under " + parentTaskId);
		...
		CalendarDate todayD = CurrentDate.getDate();
