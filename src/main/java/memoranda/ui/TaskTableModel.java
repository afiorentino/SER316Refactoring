...
public class TaskTableModel extends AbstractTreeTableModel implements TreeTableModel {
    ...
    public Object getValueAt(Object node, int column) {
        ...
        case 5:
            return getStatusString(t.getStatus(CurrentDate.getDate()));
        case 6:            
    ...
    public int getChildCount(Object parent) {
		...
		if( activeOnly() ){
			return CurrentProject.getTaskList().getActiveSubTasks(null, CurrentDate.getDate()).size();
		}
		else return CurrentProject.getTaskList().getTopLevelTasks().size();
        }
        ITask t = (ITask) parent;
        if(activeOnly()) return CurrentProject.getTaskList().getActiveSubTasks(t.getID(), CurrentDate.getDate()).size();
    ...
    public Object getChild(Object parent, int index) {
        if (parent instanceof IProject)
            if( activeOnly() ) return CurrentProject.getTaskList().getActiveSubTasks(null, CurrentDate.getDate()).toArray()[index];
	    else return CurrentProject.getTaskList().getTopLevelTasks().toArray()[index];
        ITask t = (ITask) parent;
        if(activeOnly()) return CurrentProject.getTaskList().getActiveSubTasks(t.getID(), CurrentDate.getDate()).toArray()[index];
